...
def find_height(state, col):
	...
	for row,cell in enumerate(grid[col]):
		if cell == "#":
			...
			return row
		if cell == COLOR[0] or cell == COLOR[1]:
			continue
		else:
			return row-1
	...
	if is_max_player:
		...
		for i in range(WIDTH):
			if is_playable(test_grid, i):
				...
				test_grid = matrix
				best_val = max(best_val,minimax(test_grid, False, depth+1))
				#Undo the move
				board = matrix
		return best_val
	#minimizing player
		...
		for i in range(WIDTH):
			if is_playable(test_grid, i):
				...
				best_val = min(best_val,minimax(test_grid, True, depth+1))
				#Undo the move
				board = matrix
		return best_val

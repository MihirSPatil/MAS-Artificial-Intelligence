...
class Computer_player(Player):
    ...
    def find_best_move(self, grid):
        ...
        # will map legal move states to their state values
        playable_moves = {}
        ...
        for col in xrange(GRID_WIDTH):
            if self.is_playable(col, grid):
                ...
                #print dup_board
                playable_moves[col] = -self.minimax(self.depth - 1, dup_board, human_color, MIN)
        print "playable moves", len(playable_moves)
    ...
    def minimax(self, depth, grid, curr_player_color, alpha):
        ...
        for child in playable_moves:
            if child == None:
                print("done")
            best_val = max(best_val, -self.minimax(depth - 1, child, opp_player_color, alpha))

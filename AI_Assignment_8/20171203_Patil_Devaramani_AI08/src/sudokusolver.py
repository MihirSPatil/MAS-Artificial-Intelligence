import copy, Queue
#data = [('c'+str(i)+str(j), c) for i, line in enumerate(open("50sudoku.txt",r)) for j, c in enumerate(line[0:-1]) if len(line)>8]
...
for x in range(1,51):
    printSudoku(solveDFS(readSudoku('Grid'+' '+'%02d.txt'%(x))))
#printSudoku(solveDFS(readSudoku('testdoku.txt'))
